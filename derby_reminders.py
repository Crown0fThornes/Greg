...
@command_handler.Scheduled(time="12:00", day_of_month=21)   #8am
async def town_tasks_reminder_tuesday(client):
    ...
    for family in FAMILIES_JSON:
        ...
        target_context = Context(await cur_chat.send(msg))
        await target_context.react(family["emoji"])
@command_handler.Scheduled(time="12:00", day_of_month=23)   #8am
async def town_tasks_reminder_thursday(client):
    ...
    for family in FAMILIES_JSON:
        ...
        target_context = Context(await cur_chat.send(msg))
        await target_context.react(family["emoji"])     
@command_handler.Scheduled(time="12:00", day_of_month=24)   #8am
...
@command_handler.Scheduled(time="00:00", day_of_month=25)   # 8pm
async def town_tasks_reminder_friday_final_warning(client):
    ...
    for family in FAMILIES_JSON:
        ...
        target_context = Context(await cur_chat.send(msg))
        await target_context.react(family["emoji"])
@command_handler.Scheduled(time="12:00", day_of_month=25)   #8am
